...
parfor exp_idx = 1:20
    for c_idx = 1:length(CHAN_STRS)
            ...
            if exp_idx == regparams.FIXED_RUN
                ...
                elseif strcmp(CHAN_STRS{c_idx},'ch00')
                    filename_in = fullfile(params.deconvolutionImagesDir,sprintf('%s_round%.03i_%s.tif',params.FILE_BASENAME,exp_idx,CHAN_STRS{c_idx}));
                else
                    filename_in = fullfile(params.colorCorrectionImagesDir,sprintf('%s_round%.03i_%s.tif',params.FILE_BASENAME,exp_idx,CHAN_STRS{c_idx}));
                end
            else
